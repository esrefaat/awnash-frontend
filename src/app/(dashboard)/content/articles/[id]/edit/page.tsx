...
import { authenticatedPost, authenticatedGet, authenticatedPatch, authenticatedFileUpload } from '@/lib/apiUtils';
...
const EditArticlePage: React.FC = () => {
  ...
  useEffect(() => {
    const loadArticle = async () => {
      ...
      try {
        ...
        const result = await authenticatedGet(`/content/articles/${params.id}`);
        ...
        if (article) {
          ...
          setErrors({ general: isRTL ? 'المقال غير موجود' : 'Article not found' });
        }
      } catch (error: any) {
        console.error('Error loading article:', error);
        setErrors({ 
          general: error.message || (isRTL ? 'خطأ في تحميل المقال' : 'Failed to load article')
        });
      } finally {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      // Submit to API (backend uses PATCH for updates)
      await authenticatedPatch(`/content/articles/${params.id}`, submitData);
      ...
      setErrors({ 
        general: error.message || (isRTL ? 'حدث خطأ في تحديث المقال' : 'Failed to update article')
      });
    } finally {
      setLoading(false);
    }
  ...
  if (initialLoading) {
    return (
      <div className={`min-h-screen bg-background flex items-center justify-center ${isRTL ? 'font-arabic' : 'font-montserrat'}`} dir={isRTL ? 'rtl' : 'ltr'}>
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-awnash-primary mx-auto mb-4"></div>
          <p className="text-muted-foreground">{isRTL ? 'جاري تحميل المقال...' : 'Loading article...'}</p>
        </div>
      </div>
    );
  }
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-muted-foreground">
              {isRTL ? 'تحرير مقال موجود في منصة أونش' : 'Edit an existing article for the Awnash platform'}
            </p>
          </div>
        </div>
                    ...
                    <div className="mt-2">
                      {formData.coverImageUrl ? (
                        <div className="flex items-center gap-2">
                          ...
                          />
                          <div>
                            <p className="text-sm text-green-400">✓ Cover image</p>
                            <button
                              type="button"
