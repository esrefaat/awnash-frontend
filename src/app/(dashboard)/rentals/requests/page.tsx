...
const EquipmentRequests: React.FC = () => {
  ...
  const getPriorityBadge = (priority: string, isHigh: boolean) => {
    if (isHigh) {
      return (
        <span className="px-2 py-1 bg-red-500 text-foreground rounded-full text-xs font-medium flex items-center">
        ...
        {loading && (
          <div className="bg-card rounded-xl border border-border p-12 text-center">
            <div className="flex items-center justify-center">
              ...
              <span className="text-muted-foreground">{isRTL ? 'جاري تحميل الطلبات...' : 'Loading requests...'}</span>
            </div>
          </div>
        )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
            <div className="bg-card rounded-xl border border-border p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-muted-foreground text-sm">{isRTL ? 'إجمالي الطلبات' : 'Total Requests'}</p>
            ...
            <div className="bg-card rounded-xl border border-border p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-muted-foreground text-sm">{isRTL ? 'مفتوحة' : 'Open'}</p>
            ...
            <div className="bg-card rounded-xl border border-border p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-muted-foreground text-sm">{isRTL ? 'مكتملة' : 'Fulfilled'}</p>
            ...
            <div className="bg-card rounded-xl border border-border p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-muted-foreground text-sm">{isRTL ? 'منتهية الصلاحية' : 'Expired'}</p>
            ...
            <div className="bg-card rounded-xl border border-border p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-muted-foreground text-sm">{isRTL ? 'بدون رد' : 'No Response'}</p>
                ...
                <tr>
                  <th className={cn('px-6 py-3 text-xs font-medium text-muted-foreground uppercase tracking-wider', isRTL ? 'text-right' : 'text-left')}>
                    <div className="flex items-center">
                      ...
                      <FontAwesomeIcon icon={faSort} className={cn('h-3 w-3 text-muted-foreground', isRTL ? 'mr-1' : 'ml-1')} />
                    </div>
                  </th>
                  <th className={cn('px-6 py-3 text-xs font-medium text-muted-foreground uppercase tracking-wider', isRTL ? 'text-right' : 'text-left')}>
              ...
              <tbody className="divide-y divide-border">
                {filteredRequests.map((request) => {
                  ...
                  <tr key={request.id} className="hover:bg-muted transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div>
                        <div className="text-sm text-foreground">{request.equipmentType}</div>
                        <div className="text-xs text-muted-foreground flex items-center mt-1">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-foreground">{request.count} {isRTL ? 'قطعة' : 'piece(s)'}</div>
                      <div className="text-xs text-muted-foreground">
                        {formatDate(request.startDate)} - {formatDate(request.endDate)}
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-foreground">{formatDate(request.createdAt)}</div>
                      <div className="text-xs text-muted-foreground">
