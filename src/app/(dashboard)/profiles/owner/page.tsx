...
const OwnerProfile: React.FC = () => {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-card rounded-xl border border-gray-200 dark:border-border shadow-sm">
        <div className="p-6 border-b border-gray-200 dark:border-border">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-foreground">Equipment Inventory</h3>
        </div>
          ...
          <table className="w-full">
            ...
            <tbody className="bg-white dark:bg-card divide-y divide-gray-200 dark:divide-border">
              {mockEquipmentData.map((equipment) => (
                <tr key={equipment.id} className="hover:bg-gray-50 dark:hover:bg-muted">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      ...
                      <div className={`${isRTL ? 'mr-4' : 'ml-4'}`}>
                        <div className="text-sm font-medium text-gray-900 dark:text-foreground">{equipment.name}</div>
                        <div className="text-sm text-gray-500 dark:text-muted-foreground">{equipment.location}</div>
                      </div>
                    </div>
                  </td>
  ...
  const renderBookingsTab = () => (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-card rounded-xl border border-gray-200 dark:border-border shadow-sm">
        <div className="p-6 border-b border-gray-200 dark:border-border">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-foreground">Recent Bookings as Owner</h3>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-50 dark:bg-muted">
              ...
              {mockBookingsAsOwner.map((booking) => (
                <tr key={booking.id} className="hover:bg-gray-50 dark:hover:bg-muted">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-foreground">
                    ...
                    <div className="text-xs text-gray-500 dark:text-muted-foreground">{booking.duration} days</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-foreground">
  ...
  const renderDocumentsTab = () => (
    <div className="space-y-6">
      <div className="bg-white dark:bg-card rounded-xl border border-gray-200 dark:border-border shadow-sm">
        <div className="p-6 border-b border-gray-200 dark:border-border">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-foreground">Document Verification</h3>
        </div>
        <div className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {mockDocuments.map((doc) => (
              <div key={doc.id} className="border border-gray-200 dark:border-border rounded-lg p-4">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center">
                                         <FontAwesomeIcon icon={faFileAlt} className={`h-5 w-5 text-blue-700 dark:text-blue-400 ${isRTL ? 'ml-2' : 'mr-2'}`} />
                    <h4 className="font-medium text-gray-900 dark:text-foreground">{doc.type}</h4>
                  </div>
  ...
  const renderMessagesTab = () => (
    <div className="space-y-6">
      <div className="bg-white dark:bg-card rounded-xl border border-gray-200 dark:border-border shadow-sm">
        <div className="p-6 border-b border-gray-200 dark:border-border">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-foreground">Message History</h3>
        </div>
        <div className="p-6">
          <div className="space-y-4">
            {mockMessages.map((message) => (
              <div key={message.id} className="border border-gray-200 dark:border-border rounded-lg p-4">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center">
                    <FontAwesomeIcon icon={faComments} className={`h-5 w-5 text-blue-700 dark:text-blue-400 ${isRTL ? 'ml-2' : 'mr-2'}`}/>
                    <span className="font-medium text-gray-900 dark:text-foreground">Chat with {message.with}</span>
                  </div>
                  <span className="text-sm text-gray-500 dark:text-muted-foreground">{message.timestamp}</span>
                </div>
                <div className="text-sm text-gray-600 dark:text-muted-foreground mb-2">
                  Equipment: {message.equipment}
                </div>
                <div className="text-sm text-gray-800 dark:text-foreground mb-3">
                  "{message.lastMessage}"
                </div>
            ...
            <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between">
              ...
              <div className="flex items-center mb-6 lg:mb-0">
                <div className="relative">
                  {mockOwnerData.companyLogo ? (
                    <img 
                      ...
                      alt="Company Logo" 
                      className="w-20 h-20 rounded-xl bg-gray-200 dark:bg-muted"
                    />
                  ) : (
                                       <div className="w-20 h-20 rounded-xl bg-blue-700 flex items-center justify-center">
                     <FontAwesomeIcon icon={faBuilding} className="h-10 w-10 text-foreground" />
                   </div>
                  )}
                ...
                <div className={`${isRTL ? 'mr-6' : 'ml-6'}`}>
                  <h1 className="text-3xl font-bold text-gray-900 dark:text-foreground">{mockOwnerData.companyName}</h1>
                  <p className="text-lg text-gray-600 dark:text-muted-foreground mt-1">{mockOwnerData.ownerName}</p>
                  ...
                  <div className="flex items-center mt-2">
                    <FontAwesomeIcon icon={faMapMarkerAlt} className={`h-4 w-4 ${isRTL ? 'ml-2' : 'mr-2'} text-gray-600 dark:text-muted-foreground`}/>
                    <span className="text-gray-600 dark:text-muted-foreground">{mockOwnerData.location}</span>
