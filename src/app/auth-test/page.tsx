  ...
  useEffect(() => {
    const testAuth = async () => {
      try {
        ...
        const authResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3007/v1'}/auth/me`, {
          method: 'GET',
          credentials: 'include',
        });
        ...
        if (authResponse.ok) {
          ...
          const permissionsResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3007/v1'}/permissions`, {
            method: 'GET',
            credentials: 'include',
          });
